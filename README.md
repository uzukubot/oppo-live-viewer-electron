...
- **拖拽 / 右键"打开方式"**：把图片或文件夹拖进窗口，或在系统里对图片/文件夹右键 → 打开方式 → 本应用，都会打开这张图并让侧边栏列出其所在目录。
- 流式扫描大文件夹：**列表秒出**（只读文件名，不读文件头），HDR/LIVE 徽标**异步填充**
  （后台读文件头解析，已解析的不重复解析；选中某张图时优先解析它的徽标）。
- **文件夹实时监听**：打开期间文件夹里增删图片会自动反映到列表（新增进徽标队列自动打标，
  删除从列表移除，总数同步更新）。
